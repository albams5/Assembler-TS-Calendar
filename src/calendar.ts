...
function printMonth (year: number, numberMonth:number): void {
    ...
    const firstDayAttributes: string = `class='col-start-${(month.start === 0) ? 7 : month.start} text-right bg-white px-2 border-solid border-2 rounded border-grey-400 h-[5.5rem] w-32 group cursor-pointer hover:scale-110 overflow-auto relative overflow-visible'`
    const dayAttributes = `class='text-right bg-white px-2 border-solid border-2 rounded border-grey-400 h-[5.5rem] w-32 group cursor-pointer overflow-y-auto hover:scale-110 relative overflow-x-visible'`
    const htmlDays: string = days
        .map(
            (day, index) =>
            `<li ${index === 0 ? firstDayAttributes : dayAttributes}>
                <button class="add absolute invisible group-hover:visible border-solid border-2 border-violet-300 left-0 px-1 bg-violet-100 text-violet-300" date="${month.year}-${month.id}-${day}" class="add">add</button>
                <span>${day}</span>
