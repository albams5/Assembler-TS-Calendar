...
export function setPage(): void {
  ...
  languageSelect.addEventListener("change", printToday);
  btnPrev.addEventListener("click", showPrevMonth);
  btnNext.addEventListener("click", showNextMonth);
  btnToday.addEventListener("click", printToday);
  btnAddEvent.addEventListener("click", paintDom);
  printToday();
  setInterval( checkEventReminder, 10000);
}

// this function shows the current day on the screen clicking on button "go today"
...
// this function sets style for current day
function setToday(): void {
  ...
  if (actualYear === month.year && actualMonth === month.id) {
    ...
    today?.children[1].classList.add("text-[#F3EEE7]", "rounded-full", "bg-[#AB4F65]", "px-2");
  }
}
...
function showNextMonth(): void {
  let calendar = localStorage.getItem("calendar") || '{"eventList":[], "currentMonth":{}}';
  const month = JSON.parse(calendar).currentMonth;
  const year = month.id === 11 || month.id % 11 === -1 ? month.year + 1 : month.year;

  printMonth(year, (month.id + 1) % 12);
}

// this function returns an array with names of week in locale language
...
// this function returns an object month with interface Month structure
...
// this function prints on screen one month
export function printMonth(year: number, numberMonth: number): void {
  ...
  const firstDayAttributes: string = `class='min-h-[5rem] col-start-${month.start === 0 ? 7 : month.start} text-right relative bg-white px-2 border-solid border-2 rounded border-grey-400 group cursor-pointer fade-in-right hover:bg-red-50 w-full'`;
  ...
  const htmlDays: string = days.map(
      (day, index) =>
        `<li ${index === 0 ? firstDayAttributes : dayAttributes}>
            <button class="add absolute left-2 top-1 invisible group-hover:visible" date="${month.year}-${month.id}-${day}">
                <svg class="pointer-events-none" width="20px" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g  stroke-width="2"></g><g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g><g id="SVGRepo_iconCarrier">
                        <path d="M12.75 9C12.75 8.58579 12.4142 8.25 12 8.25C11.5858 8.25 11.25 8.58579 11.25 9L11.25 11.25H9C8.58579 11.25 8.25 11.5858 8.25 12C8.25 12.4142 8.58579 12.75 9 12.75H11.25V15C11.25 15.4142 11.5858 15.75 12 15.75C12.4142 15.75 12.75 15.4142 12.75 15L12.75 12.75H15C15.4142 12.75 15.75 12.4142 15.75 12C15.75 11.5858 15.4142 11.25 15 11.25H12.75V9Z" fill="#ab4f65"></path>
                        <path  fill-rule="evenodd" clip-rule="evenodd" d="M12 1.25C6.06294 1.25 1.25 6.06294 1.25 12C1.25 17.9371 6.06294 22.75 12 22.75C17.9371 22.75 22.75 17.9371 22.75 12C22.75 6.06294 17.9371 1.25 12 1.25ZM2.75 12C2.75 6.89137 6.89137 2.75 12 2.75C17.1086 2.75 21.25 6.89137 21.25 12C21.25 17.1086 17.1086 21.25 12 21.25C6.89137 21.25 2.75 17.1086 2.75 12Z" fill="#ab4f65"></path>
                    </g>
                </svg>
            </button>
            <span>${day}</span>
